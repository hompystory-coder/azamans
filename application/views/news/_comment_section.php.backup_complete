...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body">
        ...
        <!-- 댓글 작성 폼 -->
        <?php if ($news['comment_level'] <= ($_SESSION['level'] ?? 0)): ?>
            <form id="commentForm">
                <div class="mb-3">
...
<script>
// 전역 변수
const boardId = '<?php echo $news['news_id']; ?>';
...
document.getElementById('commentForm')?.addEventListener('submit', function(e) {
    ...
    fetch(`/news/${boardId}/comment/${postUid}`, {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
...
function loadComments() {
    fetch(`/news/${boardId}/${postUid}/comments`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
...
function saveEdit(commentId) {
    ...
    fetch(`/news/${boardId}/${postUid}/comment/${commentId}/edit`, {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
...
function deleteCommentLocal(commentId) {
    ...
    // AJAX 요청
    fetch(`/news/${boardId}/${postUid}/comment/${commentId}/delete`, {
        method: 'POST'
    })
    .then(res => res.json())
    .then(data => {
