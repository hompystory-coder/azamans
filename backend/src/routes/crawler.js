// 블로그/기사 크롤링 API
import express from 'express';
import axios from 'axios';
import * as cheerio from 'cheerio';

const router = express.Router();

// 네이버 블로그 iframe URL 추출
async function getNaverBlogIframeUrl(url) {
  try {
    const response = await axios.get(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      }
    });
    
    const $ = cheerio.load(response.data);
    const iframeSrc = $('#mainFrame').attr('src');
    
    if (iframeSrc) {
      return iframeSrc.startsWith('http') ? iframeSrc : `https://blog.naver.com${iframeSrc}`;
    }
    
    return null;
  } catch (error) {
    return null;
  }
}

// POST /api/crawler/fetch - 블로그/기사 크롤링
router.post('/fetch', async (req, res) => {
  try {
    const { url } = req.body;
    
    if (!url) {
      return res.status(400).json({
        success: false,
        error: 'URL이 필요합니다.'
      });
    }
    
    console.log(`🕷️ 크롤링 시작: ${url}`);
    
    let targetUrl = url;
    
    // 네이버 블로그 iframe URL 추출
    if (url.includes('blog.naver.com') && !url.includes('PostView.naver')) {
      const iframeUrl = await getNaverBlogIframeUrl(url);
      if (iframeUrl) {
        console.log(`📍 네이버 블로그 iframe URL: ${iframeUrl}`);
        targetUrl = iframeUrl;
      }
    }
    
    // URL 페이지 가져오기
    const response = await axios.get(targetUrl, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      },
      timeout: 30000
    });
    
    const html = response.data;
    const $ = cheerio.load(html);
    
    // 제목 추출
    let title = $('meta[property="og:title"]').attr('content') ||
                $('title').text() ||
                $('h1').first().text() ||
                '';
    title = title.trim();
    
    // 본문 추출 (여러 선택자 시도)
    const contentSelectors = [
      '.se-main-container',  // 네이버 블로그 스마트에디터
      '.se-component-content',  // 네이버 블로그
      '#postViewArea',  // 네이버 블로그 (구버전)
      'article',
      '.article-content',
      '.post-content',
      '.entry-content',
      '#content',
      'main',
      '.content'
    ];
    
    let contentText = '';
    for (const selector of contentSelectors) {
      const content = $(selector).text();
      if (content && content.length > contentText.length) {
        contentText = content;
      }
    }
    
    // 네이버 블로그 특수 처리 - p 태그들 수집
    if (!contentText && targetUrl.includes('blog.naver.com')) {
      const paragraphs = [];
      $('p, .se-text-paragraph').each((i, elem) => {
        const text = $(elem).text().trim();
        if (text && text.length > 10) {
          paragraphs.push(text);
        }
      });
      contentText = paragraphs.join('\n\n');
    }
    
    // 텍스트 정리
    contentText = contentText
      .replace(/\s+/g, ' ')
      .replace(/\n+/g, '\n')
      .trim();
    
    // 이미지 추출 - 본문 컨테이너 내부의 이미지만 추출
    const images = [];
    const imageUrls = new Set(); // 중복 방지
    
    // 본문 컨테이너 선택자
    const contentContainerSelectors = [
      '.se-main-container',  // 네이버 블로그 스마트에디터
      '.se-component-content',  // 네이버 블로그
      '#postViewArea',  // 네이버 블로그 (구버전)
      'article',
      '.article-content',
      '.post-content',
      '.entry-content',
      '#content',
      'main'
    ];
    
    // 본문 컨테이너 찾기
    let $contentContainer = null;
    for (const selector of contentContainerSelectors) {
      const $container = $(selector);
      if ($container.length > 0) {
        $contentContainer = $container.first();
        console.log(`📦 본문 컨테이너 발견: ${selector}`);
        break;
      }
    }
    
    // 본문 컨테이너 내부의 이미지만 추출
    const $images = $contentContainer ? $contentContainer.find('img') : $('img');
    
    $images.each((i, elem) => {
      let src = $(elem).attr('src') || $(elem).attr('data-src') || $(elem).attr('data-lazy-src');
      
      if (src) {
        // 상대 경로를 절대 경로로 변환
        if (src.startsWith('/')) {
          const urlObj = new URL(targetUrl);
          src = `${urlObj.protocol}//${urlObj.host}${src}`;
        } else if (!src.startsWith('http')) {
          const urlObj = new URL(targetUrl);
          src = `${urlObj.protocol}//${urlObj.host}/${src}`;
        }
        
        // 네이버 블로그 이미지: 썸네일 URL 사용 (type=w966)
        if (src.includes('postfiles.pstatic.net') || src.includes('blogfiles.pstatic.net')) {
          // 기존 쿼리 파라미터 제거 후 w966 썸네일 사용
          const baseUrl = src.split('?')[0];
          src = `${baseUrl}?type=w966`;
          
          console.log(`🖼️  네이버 썸네일 이미지 URL: ${src}`);
        }
        
        // 작은 이미지 필터링 (아이콘, 이모티콘 등)
        const width = parseInt($(elem).attr('width') || '9999');
        const height = parseInt($(elem).attr('height') || '9999');
        
        // 명시적으로 작은 이미지만 제외
        if ((width > 0 && width < 50) || (height > 0 && height < 50)) {
          return; // skip very small images
        }
        
        // UI 요소 이미지 제외
        const srcLower = src.toLowerCase();
        if (srcLower.includes('icon') || 
            srcLower.includes('emoji') || 
            srcLower.includes('btn_') ||
            srcLower.includes('logo') ||
            srcLower.includes('profile') ||
            srcLower.includes('avatar') ||
            srcLower.includes('_preset_') ||
            srcLower.includes('/static/blog/') ||
            (srcLower.includes('.gif') && !srcLower.includes('postfiles'))) {
          return; // skip
        }
        
        // 네이버 블로그 UI 이미지 제외
        if (srcLower.includes('profileimage') || 
            srcLower.includes('mainthumbnailurl') ||
            srcLower.includes('blogpfthumb')) {
          return; // skip
        }
        
        // 중복 제거
        if (imageUrls.has(src)) {
          return;
        }
        
        imageUrls.add(src);
        
        // alt 텍스트
        const alt = $(elem).attr('alt') || '';
        
        // 이미지 프록시 URL 생성
        const proxyUrl = `/api/image-proxy?url=${encodeURIComponent(src)}`;
        
        images.push({
          url: src,
          proxyUrl: proxyUrl,
          alt: alt.trim()
        });
      }
    });
    
    console.log(`🖼️  본문 이미지 추출: ${images.length}개`);
    
    console.log(`✅ 크롤링 완료: 제목="${title}", 텍스트=${contentText.length}자, 이미지=${images.length}개`);
    
    res.json({
      success: true,
      data: {
        title,
        content: contentText,
        images: images.slice(0, 20), // 최대 20개
        url
      }
    });
    
  } catch (error) {
    console.error('❌ 크롤링 오류:', error);
    res.status(500).json({
      success: false,
      error: error.message || '크롤링에 실패했습니다.'
    });
  }
});

export default router;
